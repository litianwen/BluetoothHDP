...
public class BluetoothHDPService extends Service {
     ...
     * 第一步1、调用getProfileProxy(Context,BluetoothProfile.ServiceListener, int)来获取代理对象的连接。
     */
     ...
     * registerSinkAppConfiguration注册方法,主要监听蓝牙信道的状态。
     最终是调用registerAppConfiguration接口来完成
     */
    ...
    /**
     * 第二步、创建BluetoothHealthCallback回调，调用registerSinkAppConfiguration方法注册一个sink端的应用程序配置
     */
    ...
    /**
     * 第三步、将手机与健康设备配对，这一步在手机的设置中就可以完成。
     */

    /**
     * Connect channel through the Bluetooth Health API.
     第四步、使用connectChannelToSource方法来建立一个与健康设备的通信channel。
     有的设备会自动建立通信，不需要在代码中调用这个方法。第二步中的回调会指示channel的状态变化。
     ...
     * Thread to read incoming data received from the HDP device.  This sample application merely
     ...
     第五步、用ParcelFileDescriptor来读取健康设备传来的数据，并根据IEEE 11073-****协议来解析数据
     */
...
//            		fos.write(data_ABORT);
     ...
     * Disconnect channel through the Bluetooth Health API.
     第六步、通信结束后，关闭通信channel，取消第二步中的注册
      */
